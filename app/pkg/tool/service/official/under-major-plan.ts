...
import { OFFICIAL_URL, unknownResponse, createService } from "../../../../service/index.js";
...
export const getUnderMajorPlanLocal = async (): Promise<UnderMajorPlanResponse> => {
  try {
    const { data: html, status } = await request<string>(UNDER_MAJOR_PLAN_URL);
...
export const getUnderMajorPlanOnline = (): Promise<UnderMajorPlanResponse> =>
  request<UnderMajorPlanResponse>("/official/under-major-plan").then(({ data }) => data);
